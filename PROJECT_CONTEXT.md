...
2. **대시보드** — "고객사별 최신 현황"(고객사 선택과 무관하게 항상 전체 표시) → 고객사 선택 탭 → 선택 고객사의 KPI/차트/최근 업로드
3. **조직별 통계** — 고객사 선택 후 부서별 심각도 현황, 월별 오류 추이
4. **검증 이력** — 전체/고객사별 업로드 이력, 업무량 통계
5. **Rule 관리**(관리자 전용) — Rule 활성화/임계값 조정
6. 검증 상세 화면(업로드별) — AG Grid 결과 테이블(행 선택 → 처리 패널), 오류 유형별 건수 비례 막대(심각도 색상), 엑셀 내보내기, 재검증, 삭제(관리자)
